#!/usr/bin/env python3
"""
Test Quality Try-On V16: Post-CatVTON Face Swap + GFPGAN Restoration

CHANGES from v15:
- v15: Face swap without face restoration (potential quality loss)
- v16: Face swap with GFPGAN restoration (restore_face=True, restore_weight=0.5)
      → Improves face quality after swap, reduces artifacts
...
4. Phase 2B: FASHN parse realistic renders → extended agnostic masks
5. Phase 3: CatVTON-FLUX 8-angle batch with FASHN partial masks on realistic renders
6. Phase 4: Face Swap + GFPGAN Restoration on CatVTON fitted results (post-processing)
7. Phase 5: Comparison grids (5 rows: mesh, realistic, mask, fitted_before_swap, fitted_final)

GFPGAN Parameters:
- restore_face=True (enabled by default in modal_app.py)
- restore_weight=0.5 (balance between swapped face and restoration)

Output:
- output/quality_tryon_v16/mesh_{0..315}.png
- output/quality_tryon_v16/realistic_{0..315}.png
- output/quality_tryon_v16/mask_{0..315}.png
- output/quality_tryon_v16/fitted_{0..315}.png (before face swap)
- output/quality_tryon_v16/final_{0..315}.png (after face swap + GFPGAN)
- output/quality_tryon_v16/comparison_v16_part1.png
- output/quality_tryon_v16/comparison_v16_part2.png
"""
...
OUTPUT_DIR = ROOT / "output" / "quality_tryon_v16"
...
def create_extended_agnostic_mask(parsemap_b64: str, width: int, height: int) -> str:
    """Create extended agnostic mask from FASHN parse map (v13/v14/v15/v16).
...
def create_comparison_grid(
    ...
    row_labels: list[str],
    angles: list[int],
    title: str = "V16 Pipeline Comparison",
) -> Image.Image:
...
def main():
    print("=" * 80)
    print("Quality Try-On V16: Post-CatVTON Face Swap + GFPGAN Restoration")
    print("=" * 80)
    ...
    with modal_app.run():
        ...
        # ---- Phase 4: Face Swap + GFPGAN Restoration --------------------------
        print("\n  [Phase 4] Face Swap + GFPGAN Restoration on CatVTON fitted results")
        print(f"    Swapping Face.png + restoring face quality (weight=0.5)")
        ...
        if "error" in face_swap_result:
            ...
            num_restored = face_swap_result.get("num_restored", 0)
            print(f"    Face swap: {num_swapped}/{len(fitted_b64s)}, restored: {num_restored}/{len(fitted_b64s)}")
            print(f"    Total time: {t_phase4:.1f}s")
            print(f"    Cost: ${t_phase4 * H200_COST_PER_SEC:.4f}")
            for angle, status in zip(RENDER_ANGLES, swap_statuses):
                print(f"      {angle:3d}°: {status}")

        # Save final results (with face swap + GFPGAN)
        for angle, b64 in zip(RENDER_ANGLES, final_b64s):
            save_b64_image(b64, str(OUTPUT_DIR / f"final_{angle}.png"))
        print("  Final results (after face swap + GFPGAN) saved")
    ...
    grid1 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN Depth", "Ext Mask", "CatVTON", "Face+GFPGAN"],
        angles=RENDER_ANGLES[:4],
        title="V16 Post-CatVTON Face Swap + GFPGAN (0-135 deg)",
    )
    grid1_path = OUTPUT_DIR / "comparison_v16_part1.png"
    ...
    grid2 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN Depth", "Ext Mask", "CatVTON", "Face+GFPGAN"],
        angles=RENDER_ANGLES[4:],
        title="V16 Post-CatVTON Face Swap + GFPGAN (180-315 deg)",
    )
    grid2_path = OUTPUT_DIR / "comparison_v16_part2.png"
    ...
    print(f"  Phase 4    (Face Swap + GFPGAN):             {t_phase4:7.1f}s  ${t_phase4 * H200_COST_PER_SEC:.4f}")
    print(f"  {'─' * 55}")
    ...
    print("\n" + "=" * 80)
    print("V16 QUALITY TEST COMPLETE")
    print("=" * 80)
    ...
    print()
    print("V16 Post-CatVTON Face Swap + GFPGAN pipeline validation complete.")
    print("=" * 80)
    ...
    return {
        ...
        "phase_times": {
            ...
            "phase4_face_swap_gfpgan": t_phase4,
        },
        "face_swap": {
            "num_swapped": face_swap_result.get("num_swapped", 0) if "error" not in face_swap_result else 0,
            "num_restored": face_swap_result.get("num_restored", 0) if "error" not in face_swap_result else 0,
