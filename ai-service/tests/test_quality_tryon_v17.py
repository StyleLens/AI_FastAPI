#!/usr/bin/env python3
"""
Test Quality Try-On V17: ControlNet Depth + PuLID for FLUX — Face Identity

CHANGES from v13 → v17:
- v13: ControlNet Depth only → no face identity in generated images
- v14-v16: InSwapper post-processing → severe ghosting artifacts (dead end)
- v17: PuLID injects face identity during generation (denoising stage)
  → No post-processing, no ghosting, natural face integration
...
- id_weight: 1.0 (identity strength, 0.0-1.0)
- start_step: 4 (begin face injection after structure is established)
- Multi-reference: averaged embeddings from all face photos

Output:
- output/quality_tryon_v17/mesh_{0..315}.png
- output/quality_tryon_v17/realistic_{0..315}.png (WITH face identity)
- output/quality_tryon_v17/mask_{0..315}.png
- output/quality_tryon_v17/fitted_{0..315}.png
- output/quality_tryon_v17/comparison_v17_part1.png
- output/quality_tryon_v17/comparison_v17_part2.png
"""
...
OUTPUT_DIR = ROOT / "output" / "quality_tryon_v17"
...
# PuLID parameters
PULID_ID_WEIGHT = 1.0    # Identity strength (0.0-1.0)
PULID_START_STEP = 4     # Begin face injection after step 4
...
def create_comparison_grid(
    ...
    row_labels: list[str],
    angles: list[int],
    title: str = "V17 Pipeline Comparison",
) -> Image.Image:
...
def main():
    print("=" * 80)
    print("Quality Try-On V17: ControlNet Depth + PuLID Face Identity + CatVTON")
    print("=" * 80)
    ...
    print(f"  PuLID: id_weight={PULID_ID_WEIGHT}, start_step={PULID_START_STEP}")
    ...
    with modal_app.run():
        ...
        print("\n  [Phase 1.5] Mesh → Realistic + PuLID Face Identity")
        print(f"    guidance=3.5, steps=28, id_weight={PULID_ID_WEIGHT}, start_step={PULID_START_STEP}")
    ...
    grid1 = create_comparison_grid(
        ...
        title=f"V17 PuLID Face ID (id={PULID_ID_WEIGHT}, step={PULID_START_STEP}) — 0-135 deg",
    )
    grid1_path = OUTPUT_DIR / "comparison_v17_part1.png"
    ...
    grid2 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN+PuLID", "Ext Mask", "CatVTON", "Face Ref"],
        angles=RENDER_ANGLES[4:],
        title=f"V17 PuLID Face ID (id={PULID_ID_WEIGHT}, step={PULID_START_STEP}) — 180-315 deg",
    )
    grid2_path = OUTPUT_DIR / "comparison_v17_part2.png"
    ...
    print("\n" + "=" * 80)
    print("V17 QUALITY TEST COMPLETE")
    print("=" * 80)
    print(f"  Output directory: {OUTPUT_DIR}")
    print(f"  Total files: {len(output_files)}")
    print(f"  PuLID params: id_weight={PULID_ID_WEIGHT}, start_step={PULID_START_STEP}")
    ...
    print()
    print("V17 PuLID Face Identity pipeline validation complete.")
