#!/usr/bin/env python3
"""
v26 Quality Test — Enhanced Turtle Neck Fix + Photorealistic

User feedback on v25: "이번 결과도 너무 거북목이야"

Changes from v25:
  1. Turtle neck correction: 120% overcorrection + 15% Z-compression
  2. Transition starts at H=0.78 (earlier, smoother blend)
  3. Same prompt/guidance as v25 (guidance=7.0, photorealistic)
  4. Negative prompt reinforced with "forward head, chin jutting out"
...
V25_DIR = ROOT / "tests" / "v25_posture"
OUTPUT_DIR = ROOT / "tests" / "v26_neckfix"
...
def main():
    print("=" * 80)
    print("v26 Quality Test — Enhanced Turtle Neck Fix")
    print("  120% overcorrection + 15% Z-compression + H=0.78 blend start")
    print("=" * 80)
    ...
    # Phase 1R: New renderer with enhanced neck correction
    print("\n[Phase 1R] CPU Mesh Rendering (v3: enhanced neck fix)...")
    t1r_start = time.time()
    render_b64s = []
    ...
    # Phase 1.5: SDXL with reinforced anti-turtle-neck prompt
    v26_prompt = (
        ...
        "realistic skin texture, natural calm expression"
    )
    v26_negative = (
    ...
    print("\n" + "=" * 80)
    print("[Phase 1.5] SDXL Photorealistic (guidance=7.0, enhanced neck fix)")
    print("=" * 80)
    ...
    with modal_app.run():
        realistic_result = run_mesh_to_realistic.remote(
            ...
            prompt_template=v26_prompt,
            negative_prompt_override=v26_negative,
        )
    ...
    # Comparison grids: v25 vs v26
    print("\n[Phase 4] Creating comparison grids...")

    v26_r = [base64_to_pil(b) for b in realistic_b64s]
    v26_f = [base64_to_pil(b) for b in fitted_b64s]
    depth_imgs = [Image.open(OUTPUT_DIR / f"mesh_{angle_label(a)}.png") for a in ANGLES_16]

    has_v25 = (V25_DIR / "realistic_0.png").exists()
    hero_angles = [0, 45, 90, 180, 270, 315]
    hero_idx = [ANGLES_16.index(a) for a in hero_angles]

    if has_v25:
        v25_r = [Image.open(V25_DIR / f"realistic_{angle_label(a)}.png") for a in ANGLES_16]
        v25_f = [Image.open(V25_DIR / f"fitted_{angle_label(a)}.png") for a in ANGLES_16]
        ...
        hero_grid = create_comparison_grid(
            ...
            ["v26 Depth", "v25 Realistic", "v26 Realistic",
             "v25 Fitted", "v26 Fitted"],
            hero_angles,
            "v25 vs v26 — Enhanced Neck Fix",
        )
    else:
        hero_grid = create_comparison_grid(
            [[depth_imgs[i] for i in hero_idx],
    ...
    for part, sl, angles_sl, suffix in [
        ...
        grid = create_comparison_grid(
            [depth_imgs[sl], v26_r[sl], v26_f[sl]],
    ...
    report = {
        "test_name": "v26_enhanced_neck_fix",
        "timestamp": timestamp,
        "user_feedback": "v25도 거북목 → 120% overcorrection + Z-compression",
        "changes_from_v25": [
            "Turtle neck: 120% overcorrection (was 85%)",
            "Z-compression: 15% at head top (flatten neck/head Z spread)",
            "Transition from H=0.78 with smooth ease-in-out",
            "Negative prompt: 'turtle neck, forward head posture, chin jutting out'",
            "Positive prompt: 'head directly above shoulders, chin level, straight vertical neck'",
        ],
        "parameters": {
            "renderer": {"overcorrect": 1.20, "z_compress": 0.15, "blend_start": 0.78},
    ...
    print("\n" + "=" * 80)
    print("v26 Enhanced Neck Fix Test Complete")
    print("=" * 80)
    return report
