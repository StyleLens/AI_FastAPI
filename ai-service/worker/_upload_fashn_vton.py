"""Upload FASHN VTON v1.5 to Modal Volume.

FASHN VTON v1.5 is the Apache 2.0 licensed virtual try-on model.
It replaces CatVTON-FLUX + FLUX.1-Fill-dev + FASHN Parser (all non-commercial)
with a single maskless try-on model.

Key advantages:
- Maskless: no human parsing / agnostic mask needed
- 972M params, ~2GB weights (vs ~52GB for CatVTON stack)
- Commercially validated on the FASHN platform
~2GB total.
"""
import modal

app = modal.App("stylelens-upload-fashn-vton")
...
def download_fashn_vton():
    """Download FASHN VTON v1.5 to Volume."""
    import os, time
    from huggingface_hub import snapshot_download

    t0 = time.time()

    target_dir = "/models/fashn_vton"
    ...
    print("Downloading FASHN VTON v1.5 to Volume...")
    print("This is the Apache 2.0 maskless virtual try-on model.")
    print("Replaces CatVTON-FLUX + FLUX.1-Fill-dev + FASHN Parser.")
...
def main():
    import json
    result = download_fashn_vton.remote()
