#!/usr/bin/env python3
"""
Test Quality Try-On V14: Face Swap + Gray T-shirt Prompt + Extended FASHN Mask + CatVTON

CHANGES from v13:
- v13: Gray t-shirt prompt + extended mask (dress/belt) → 4-6/8 successful garment transfer
- v14 adds Phase 1.6: Face Swap (InsightFace InSwapper)
  1. After ControlNet Depth realistic renders, swap Face.png onto each render
  2. Use face-swapped renders for FASHN parsing and CatVTON
  3. Preserves target face identity throughout virtual try-on
  4. 5-row comparison grid (mesh, realistic, faceswap, mask, fitted)

Pipeline:
1. Phase 0: Load test images + mesh renders from E2E output
2. Phase 1.5: Mesh -> Realistic (ControlNet Depth, gray t-shirt prompt)
3. Phase 1.6: Face Swap (InsightFace InSwapper, swap Face.png onto each render) ← NEW
4. Phase 2A: Garment segmentation (SAM3, reuse if available)
5. Phase 2B: FASHN parse face-swapped renders → extended agnostic masks
6. Phase 3: CatVTON-FLUX 8-angle batch with FASHN partial masks
7. Comparison grids (5 rows: mesh, realistic, faceswap, mask, fitted)

Output:
- output/quality_tryon_v14/mesh_{0..315}.png
- output/quality_tryon_v14/realistic_{0..315}.png
- output/quality_tryon_v14/faceswap_{0..315}.png
- output/quality_tryon_v14/mask_{0..315}.png
- output/quality_tryon_v14/fitted_{0..315}.png
- output/quality_tryon_v14/comparison_v14_part1.png
- output/quality_tryon_v14/comparison_v14_part2.png
"""
...
OUTPUT_DIR = ROOT / "output" / "quality_tryon_v14"
...
def create_extended_agnostic_mask(parsemap_b64: str, width: int, height: int) -> str:
    """Create extended agnostic mask from FASHN parse map (v13/v14).
...
def create_comparison_grid(
    ...
    row_labels: list[str],
    angles: list[int],
    title: str = "V14 Pipeline Comparison",
) -> Image.Image:
...
def main():
    print("=" * 80)
    print("Quality Try-On V14: Face Swap + Gray T-shirt + Extended Mask + CatVTON")
    print("=" * 80)
    ...
    # ---- GPU Phases ---------------------------------------------------------
    print("\n" + "=" * 80)
    print("[Phase 1.5 + 1.6 + 2A + 2B + 3] Running GPU phases in single Modal session")
    ...
    with modal_app.run():
        ...
        # ---- Phase 1.6: Face Swap (InsightFace InSwapper) -------------------
        print("\n  [Phase 1.6] Face Swap (InsightFace InSwapper)")
        print(f"    Swapping Face.png onto {len(realistic_b64s)} realistic renders")

        t_phase16_start = time.time()

        face_swap_result = run_face_swap.remote(
            source_face_b64=face_b64,
            target_images_b64=realistic_b64s,
            angles=RENDER_ANGLES,
        )

        t_phase16 = time.time() - t_phase16_start

        if "error" in face_swap_result:
            print(f"    ERROR: {face_swap_result['error']}")
            print("    Continuing with original realistic renders (no face swap)")
            faceswapped_b64s = realistic_b64s
            t_phase16 = 0.0
        else:
            faceswapped_b64s = face_swap_result["swapped_b64"]
            swap_statuses = face_swap_result["swap_status"]
            num_swapped = face_swap_result["num_swapped"]
            print(f"    Face swap: {num_swapped}/{len(realistic_b64s)} angles swapped in {t_phase16:.1f}s")
            print(f"    Cost: ${t_phase16 * H200_COST_PER_SEC:.4f}")
            for angle, status in zip(RENDER_ANGLES, swap_statuses):
                print(f"      {angle:3d}°: {status}")

        # Save face-swapped renders
        for angle, b64 in zip(RENDER_ANGLES, faceswapped_b64s):
            save_b64_image(b64, str(OUTPUT_DIR / f"faceswap_{angle}.png"))
        ...
        print("\n  [Phase 2B] FASHN parse face-swapped renders → extended agnostic masks")
        print("    Classes: upper_clothes(4) + dress(7) + belt(8) + arms(14,15)")
        masks_b64 = []
        t_phase2b_start = time.time()

        for idx, (render_b64, angle) in enumerate(zip(faceswapped_b64s, RENDER_ANGLES)):
            print(f"    Parsing faceswap_{angle}...", end=" ")
    ...
    faceswap_pils = [base64_to_pil(b64) for b64 in faceswapped_b64s]
    ...
    grid1 = create_comparison_grid(
        rows_data=[mesh_pils[:4], realistic_pils[:4], faceswap_pils[:4], mask_pils[:4], fitted_pils[:4]],
        row_labels=["Raw Mesh", "CN Depth", "Face Swap", "Ext Mask", "CatVTON"],
        angles=RENDER_ANGLES[:4],
        title="V14 Face Swap + Gray T-shirt + Extended Mask + CatVTON (0-135 deg)",
    )
    grid1_path = OUTPUT_DIR / "comparison_v14_part1.png"
    ...
    grid2 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN Depth", "Face Swap", "Ext Mask", "CatVTON"],
        angles=RENDER_ANGLES[4:],
        title="V14 Face Swap + Gray T-shirt + Extended Mask + CatVTON (180-315 deg)",
    )
    grid2_path = OUTPUT_DIR / "comparison_v14_part2.png"
    grid2.save(grid2_path)
    print(f"  Grid Part 2 saved: {grid2_path}")

    # ---- Cost summary -------------------------------------------------------
    total_gpu_time = t_phase15 + t_phase16 + t_phase2a + t_phase2b + t_phase3
    ...
    print(f"  Phase 1.6  (Face Swap, 8 angles):        {t_phase16:7.1f}s  ${t_phase16 * H200_COST_PER_SEC:.4f}")
    ...
    print("\n" + "=" * 80)
    print("V14 QUALITY TEST COMPLETE")
    print("=" * 80)
    ...
    print(f"  Face-swapped:      faceswap_{{0,45,...,315}}.png")
    ...
    print()
    print("V14 Face Swap + Gray T-shirt + Extended Mask pipeline validation complete.")
    print("=" * 80)
