#!/usr/bin/env python3
"""
Test Quality Try-On V17C: High PuLID Weight Strategy for Maximum Face Dominance

CHANGES from v17b → v17c:
- v17b: id_weight=1.3, start_step=0, guidance=4.0, controlnet_scale=0.5
  → Still showing ControlNet structural over-constraint
- v17c: HIGH PULID WEIGHT STRATEGY:
  - id_weight=2.5 (MUCH HIGHER for face dominance, was 1.3)
  - start_step=0 (same, maximum face identity fidelity)
  - guidance=4.0 (same, official PuLID default)
  - controlnet_conditioning_scale=0.3 (LOWER to let PuLID dominate, was 0.5)

Strategy:
- Make PuLID the dominant force in face generation
- ControlNet only guides rough pose/body structure
- Higher id_weight ensures face identity overrides ControlNet depth prior
- Lower controlnet_scale gives PuLID maximum freedom in face region

Expected improvements over v17b:
- Much stronger face identity preservation
- Less ControlNet-induced face distortion
- More natural face expression matching reference
- Better handling of face details (eyes, nose, mouth shape)

Pipeline:
1. Phase 0: Load test images + mesh renders + face reference photos
2. Phase 1.5: Mesh → Realistic + Face ID (ControlNet Depth + HIGH PuLID)
...
Output:
- output/quality_tryon_v17c/mesh_{0..315}.png
- output/quality_tryon_v17c/realistic_{0..315}.png (WITH high PuLID weight)
- output/quality_tryon_v17c/mask_{0..315}.png
- output/quality_tryon_v17c/fitted_{0..315}.png
- output/quality_tryon_v17c/comparison_v17c_part1.png
- output/quality_tryon_v17c/comparison_v17c_part2.png
"""
...
OUTPUT_DIR = ROOT / "output" / "quality_tryon_v17c"
...
# HIGH PULID WEIGHT STRATEGY (v17c)
PULID_ID_WEIGHT = 2.5             # MUCH HIGHER for face dominance (was 1.3 in v17b, 1.0 in v17)
PULID_START_STEP = 0              # Begin face injection from step 0 (same as v17b)
PULID_GUIDANCE = 4.0              # Official PuLID default for FLUX (same as v17b)
CONTROLNET_CONDITIONING_SCALE = 0.3  # LOWER to let PuLID dominate (was 0.5 in v17b, 0.7 in v17)
...
def create_comparison_grid(
    ...
    row_labels: list[str],
    angles: list[int],
    title: str = "V17C Pipeline Comparison",
) -> Image.Image:
...
def main():
    print("=" * 80)
    print("Quality Try-On V17C: HIGH PuLID WEIGHT STRATEGY + CatVTON")
    print("=" * 80)
    print("\nHIGH PULID WEIGHT PARAMETERS:")
    print(f"  id_weight: {PULID_ID_WEIGHT} (was 1.3 in v17b, 1.0 in v17)")
    print(f"  start_step: {PULID_START_STEP} (same as v17b)")
    print(f"  guidance: {PULID_GUIDANCE} (same as v17b)")
    print(f"  controlnet_scale: {CONTROLNET_CONDITIONING_SCALE} (was 0.5 in v17b, 0.7 in v17)")
    print("\nStrategy: Make PuLID dominate face generation, ControlNet only guides rough pose")
    print("Expected: Much stronger face identity preservation with minimal ControlNet distortion")
    ...
    print(f"  PuLID HIGH WEIGHT: id_weight={PULID_ID_WEIGHT}, start_step={PULID_START_STEP}")
    ...
    with modal_app.run():
        ...
        print("\n  [Phase 1.5] Mesh → Realistic + HIGH WEIGHT PuLID Face Identity")
        ...
        print(f"    face refs: {len(face_b64s)} images → averaged identity embedding")
    ...
    grid1 = create_comparison_grid(
        ...
        title=f"V17C High PuLID (id={PULID_ID_WEIGHT}, step={PULID_START_STEP}, g={PULID_GUIDANCE}, cn={CONTROLNET_CONDITIONING_SCALE}) — 0-135 deg",
    )
    grid1_path = OUTPUT_DIR / "comparison_v17c_part1.png"
    ...
    grid2 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN+PuLID", "Ext Mask", "CatVTON", "Face Ref"],
        angles=RENDER_ANGLES[4:],
        title=f"V17C High PuLID (id={PULID_ID_WEIGHT}, step={PULID_START_STEP}, g={PULID_GUIDANCE}, cn={CONTROLNET_CONDITIONING_SCALE}) — 180-315 deg",
    )
    grid2_path = OUTPUT_DIR / "comparison_v17c_part2.png"
    ...
    print("\n" + "=" * 80)
    print("V17C HIGH PULID WEIGHT TEST COMPLETE")
    print("=" * 80)
    print(f"  Output directory: {OUTPUT_DIR}")
    print(f"  Total files: {len(output_files)}")
    print("\n  HIGH PULID WEIGHT PARAMETERS:")
    print(f"    id_weight={PULID_ID_WEIGHT} (vs 1.3 in v17b, 1.0 in v17)")
    print(f"    start_step={PULID_START_STEP} (same as v17b)")
    print(f"    guidance={PULID_GUIDANCE} (same as v17b)")
    print(f"    controlnet_scale={CONTROLNET_CONDITIONING_SCALE} (vs 0.5 in v17b, 0.7 in v17)")
    ...
    v17b_realistic = ROOT / "output" / "quality_tryon_v17b" / "realistic_0.png"
    if v17b_realistic.exists():
        print("\n  Opening v17b realistic_0.png for side-by-side comparison...")
        os.system(f"open '{v17b_realistic}'")

    print()
    print("V17C High PuLID Weight pipeline validation complete.")
    print("Compare realistic_0.png face with face_reference_1.png for identity match.")
    print("Expected: MUCH stronger face identity vs v17b due to high PuLID weight strategy.")
    print("Strategy: PuLID dominates face (id=2.5), ControlNet guides pose only (scale=0.3).")
    print("=" * 80)
