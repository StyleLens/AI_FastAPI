#!/usr/bin/env python3
"""
Test Quality Try-On V17B: Optimized PuLID Parameters for Maximum Face Fidelity

CHANGES from v17 → v17b:
- v17: id_weight=1.0, start_step=4, guidance=3.5, controlnet_scale=0.7
  → Suboptimal parameters (start_step=4 delays face injection)
- v17b: OPTIMIZED PULID PARAMETERS based on official defaults:
  - id_weight=1.3 (compensates for ControlNet dilution, was 1.0)
  - start_step=0 (maximum face identity fidelity, official default, was 4)
  - guidance=4.0 (official PuLID default for FLUX, was 3.5)
  - controlnet_conditioning_scale=0.5 (gives PuLID more room, was 0.7)

Research findings:
- Official PuLID-FLUX uses start_step=0 for full identity preservation
- Higher id_weight compensates for ControlNet's structural constraints
- Lower controlnet_scale gives PuLID more freedom in face region
- guidance=4.0 is PuLID's tested default for FLUX models
...
- Stronger face identity preservation (start_step=0)
- Better balance between structure and identity (optimized weights)
- More natural face integration without ControlNet over-constraint

Output:
- output/quality_tryon_v17b/mesh_{0..315}.png
- output/quality_tryon_v17b/realistic_{0..315}.png (WITH optimized face identity)
- output/quality_tryon_v17b/mask_{0..315}.png
- output/quality_tryon_v17b/fitted_{0..315}.png
- output/quality_tryon_v17b/comparison_v17b_part1.png
- output/quality_tryon_v17b/comparison_v17b_part2.png
"""
...
OUTPUT_DIR = ROOT / "output" / "quality_tryon_v17b"
...
# OPTIMIZED PuLID parameters (v17b)
PULID_ID_WEIGHT = 1.3             # Identity strength (compensates for ControlNet)
PULID_START_STEP = 0              # Begin face injection from step 0 (official default)
PULID_GUIDANCE = 4.0              # Official PuLID default for FLUX
CONTROLNET_CONDITIONING_SCALE = 0.5  # Lower value gives PuLID more room
...
def create_comparison_grid(
    ...
    row_labels: list[str],
    angles: list[int],
    title: str = "V17B Pipeline Comparison",
) -> Image.Image:
...
def main():
    print("=" * 80)
    print("Quality Try-On V17B: OPTIMIZED PuLID Parameters + CatVTON")
    print("=" * 80)
    print("\nOPTIMIZED PARAMETERS:")
    print(f"  id_weight: {PULID_ID_WEIGHT} (was 1.0 in v17)")
    print(f"  start_step: {PULID_START_STEP} (was 4 in v17, official default is 0)")
    print(f"  guidance: {PULID_GUIDANCE} (was 3.5 in v17, official default is 4.0)")
    print(f"  controlnet_scale: {CONTROLNET_CONDITIONING_SCALE} (was 0.7 in v17)")
    print("\nExpected: Stronger face identity preservation with better structure balance")
    ...
    print(f"  PuLID OPTIMIZED: id_weight={PULID_ID_WEIGHT}, start_step={PULID_START_STEP}")
    ...
    with modal_app.run():
        ...
        print("\n  [Phase 1.5] Mesh → Realistic + OPTIMIZED PuLID Face Identity")
    ...
    grid1 = create_comparison_grid(
        ...
        title=f"V17B Optimized PuLID (id={PULID_ID_WEIGHT}, step={PULID_START_STEP}, g={PULID_GUIDANCE}, cn={CONTROLNET_CONDITIONING_SCALE}) — 0-135 deg",
    )
    grid1_path = OUTPUT_DIR / "comparison_v17b_part1.png"
    ...
    grid2 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN+PuLID", "Ext Mask", "CatVTON", "Face Ref"],
        angles=RENDER_ANGLES[4:],
        title=f"V17B Optimized PuLID (id={PULID_ID_WEIGHT}, step={PULID_START_STEP}, g={PULID_GUIDANCE}, cn={CONTROLNET_CONDITIONING_SCALE}) — 180-315 deg",
    )
    grid2_path = OUTPUT_DIR / "comparison_v17b_part2.png"
    ...
    print("\n" + "=" * 80)
    print("V17B OPTIMIZED PULID TEST COMPLETE")
    print("=" * 80)
    print(f"  Output directory: {OUTPUT_DIR}")
    print(f"  Total files: {len(output_files)}")
    print("\n  OPTIMIZED PARAMETERS:")
    print(f"    id_weight={PULID_ID_WEIGHT} (vs 1.0 in v17)")
    print(f"    start_step={PULID_START_STEP} (vs 4 in v17, official default)")
    print(f"    guidance={PULID_GUIDANCE} (vs 3.5 in v17, official default)")
    print(f"    controlnet_scale={CONTROLNET_CONDITIONING_SCALE} (vs 0.7 in v17)")
    ...
    print()
    print("V17B Optimized PuLID pipeline validation complete.")
    print("Compare realistic_0.png face with face_reference_1.png for identity match.")
    print("Expected: Stronger face identity vs v17 due to optimized parameters.")
    print("=" * 80)
