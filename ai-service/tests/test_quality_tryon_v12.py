#!/usr/bin/env python3
"""
Test Quality Try-On V12: CN Depth (Clothed Prompt) + FASHN Masks + CatVTON

CRITICAL CHANGE from v11:
- v11: Prompt generated UNCLOTHED person → FASHN cannot detect upper_clothes
  → agnostic mask empty or wrong → CatVTON produces artifacts
- v12: Prompt now generates CLOTHED person (white t-shirt + jeans)
  → FASHN reliably detects upper_clothes at all angles
  → CatVTON replaces the white t-shirt with the target garment
  → Mask logic (upper_clothes + arms) is IDENTICAL to v11

Pipeline:
1. Phase 0: Load test images + mesh renders from E2E output
2. Phase 1.5: Mesh -> Realistic (ControlNet Depth, same as v10)
3. Phase 2A: Garment segmentation (SAM3, reuse if available)
4. Phase 2B: FASHN parse each realistic render → agnostic masks (NEW)
5. Phase 3: CatVTON-FLUX 8-angle batch with FASHN partial masks
6. Comparison grids (4 rows: mesh, realistic, mask, fitted)

Output:
- output/quality_tryon_v12/mesh_{0..315}.png
- output/quality_tryon_v12/realistic_{0..315}.png
- output/quality_tryon_v12/mask_{0..315}.png          (NEW: FASHN agnostic masks)
- output/quality_tryon_v12/fitted_{0..315}.png
- output/quality_tryon_v12/comparison_v12_part1.png
- output/quality_tryon_v12/comparison_v12_part2.png
"""
...
OUTPUT_DIR = ROOT / "output" / "quality_tryon_v12"
...
def create_agnostic_mask_from_parsemap(parsemap_b64: str, width: int, height: int) -> str:
    """Create CatVTON agnostic mask from FASHN parse map.
    ...
    White(255) = upper_clothes(4) + left_arm(14) + right_arm(15) → inpaint
...
def create_comparison_grid(
    ...
    row_labels: list[str],
    angles: list[int],
    title: str = "V12 Pipeline Comparison",
) -> Image.Image:
...
def main():
    print("=" * 80)
    print("Quality Try-On V12: CN Depth (Clothed Prompt) + FASHN Masks + CatVTON")
    print("=" * 80)
    ...
    with modal_app.run():
        ...
        print(f"    prompt: wearing plain white t-shirt + jeans (v12 fix)")
        ...
        # ---- Phase 2B: FASHN parse → agnostic masks (NEW!) -------------------
        print("\n  [Phase 2B] FASHN parse realistic renders → agnostic masks")
        masks_b64 = []
        ...
        for idx, (realistic_b64, angle) in enumerate(zip(realistic_b64s, RENDER_ANGLES)):
            ...
            if "error" in parse_result:
                ...
                agnostic_mask = create_agnostic_mask_from_parsemap(
                ...
                # Save parse map too for debugging
        ...
        # ---- Phase 3: CatVTON-FLUX with FASHN masks --------------------------
        print("\n  [Phase 3] CatVTON-FLUX 8-angle batch (FASHN partial masks)")
        print(f"    guidance = 30.0, num_steps = 30")
        print(f"    Masks: FASHN agnostic (upper_clothes+arms), NOT full white")
    ...
    grid1 = create_comparison_grid(
        rows_data=[mesh_pils[:4], realistic_pils[:4], mask_pils[:4], fitted_pils[:4]],
        row_labels=["Raw Mesh", "CN Depth", "FASHN Mask", "CatVTON"],
        angles=RENDER_ANGLES[:4],
        title="V12 CN Depth (Clothed Prompt) + FASHN Mask + CatVTON (0-135 deg)",
    )
    grid1_path = OUTPUT_DIR / "comparison_v12_part1.png"
    ...
    grid2 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN Depth", "FASHN Mask", "CatVTON"],
        angles=RENDER_ANGLES[4:],
        title="V12 CN Depth (Clothed Prompt) + FASHN Mask + CatVTON (180-315 deg)",
    )
    grid2_path = OUTPUT_DIR / "comparison_v12_part2.png"
    ...
    print("\n" + "=" * 80)
    print("V12 QUALITY TEST COMPLETE")
    print("=" * 80)
    ...
    print(f"  FASHN masks:       mask_{{0,45,...,315}}.png")
    ...
    print()
    print("V12 FASHN Partial Mask pipeline validation complete.")
    print("=" * 80)
