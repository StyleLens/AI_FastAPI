#!/usr/bin/env python3
"""
Test Quality Try-On V13: Gray T-shirt Prompt + Extended FASHN Mask + CatVTON

CHANGES from v12:
- v12: White t-shirt prompt → CatVTON struggles to replace white (too strong prior)
  → Only 3-4/8 angles successfully transferred garment
- v13 fixes:
  1. Prompt: "plain gray t-shirt" instead of "plain white t-shirt"
     → Neutral gray is easier for CatVTON to overwrite with target garment
  2. Mask: Added dress(7) + belt(8) to inpaint classes
     → FASHN sometimes misclassifies t-shirt as dress at non-frontal angles
  3. Same pipeline structure as v12 otherwise
...
4. Phase 2B: FASHN parse each realistic render → extended agnostic masks
5. Phase 3: CatVTON-FLUX 8-angle batch with FASHN partial masks
6. Comparison grids (4 rows: mesh, realistic, mask, fitted)

Output:
- output/quality_tryon_v13/mesh_{0..315}.png
- output/quality_tryon_v13/realistic_{0..315}.png
- output/quality_tryon_v13/mask_{0..315}.png
- output/quality_tryon_v13/fitted_{0..315}.png
- output/quality_tryon_v13/comparison_v13_part1.png
- output/quality_tryon_v13/comparison_v13_part2.png
"""
...
OUTPUT_DIR = ROOT / "output" / "quality_tryon_v13"
...
def create_extended_agnostic_mask(parsemap_b64: str, width: int, height: int) -> str:
    """Create extended agnostic mask from FASHN parse map (v13).
    ...
    v13 extended mask (vs v12):
    White(255) = upper_clothes(4) + left_arm(14) + right_arm(15)
               + dress(7) + belt(8)  ← NEW: FASHN misclassifies these at side angles
...
def create_comparison_grid(
    ...
    row_labels: list[str],
    angles: list[int],
    title: str = "V13 Pipeline Comparison",
) -> Image.Image:
...
def main():
    print("=" * 80)
    print("Quality Try-On V13: Gray T-shirt Prompt + Extended Mask + CatVTON")
    print("=" * 80)
    ...
    grid1 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN Depth", "Ext Mask", "CatVTON"],
        angles=RENDER_ANGLES[:4],
        title="V13 Gray T-shirt + Extended Mask + CatVTON (0-135 deg)",
    )
    grid1_path = OUTPUT_DIR / "comparison_v13_part1.png"
    ...
    grid2 = create_comparison_grid(
        ...
        row_labels=["Raw Mesh", "CN Depth", "Ext Mask", "CatVTON"],
        angles=RENDER_ANGLES[4:],
        title="V13 Gray T-shirt + Extended Mask + CatVTON (180-315 deg)",
    )
    grid2_path = OUTPUT_DIR / "comparison_v13_part2.png"
    ...
    print("\n" + "=" * 80)
    print("V13 QUALITY TEST COMPLETE")
    print("=" * 80)
    ...
    print()
    print("V13 Gray T-shirt + Extended Mask pipeline validation complete.")
    print("=" * 80)
